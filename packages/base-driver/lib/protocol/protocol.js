...
function getLogger(driver, sessionId = null) {
  ...
  let logPrefix = dstDriver.constructor
    ? `${dstDriver.constructor.name}@${node
        .getObjectId(dstDriver)
        .substring(0, 8)}`
    : 'AppiumDriver';
  if (sessionId) {
...
function makeArgs(requestParams, jsonObj, payloadParams, protocol) {
  ...
  if (_.isFunction(payloadParams.makeArgs)) {
    ...
    if (payloadParams.optional) {
      args = args.concat(
        _.flatten(payloadParams.optional).map((p) => jsonObj[p])
      );
    }
  }
...
function routeConfiguringFunction(driver) {
  ...
  if (!(driver.executeCommand || driver.execute)) {
    throw new Error(
      'Drivers must implement `executeCommand` or `execute` method'
    );
  }
  ...
  return function addRoutes(
    app,
    {basePath = DEFAULT_BASE_PATH, extraMethodMap = {}}
  ) {
    ...
    try {
      ...
      if (
        ...
        if (
          ...
          driver.pluginsToHandleCmd(spec.command, req.params.sessionId)
            .length === 0
        ) {
      ...
      currentProtocol =
        extractProtocol(driver, req.params.sessionId) || currentProtocol;
      ...
      getLogger(driver, req.params.sessionId || newSessionId).debug(
        `Responding ` +
          `to client with driver.${spec.command}() result: ${_.truncate(
