...
export class InvalidArgumentError extends ProtocolError {
  ...
  constructor(err) {
    super(
      err ||
        'The arguments passed to the command are either invalid or malformed',
...
export class NoAlertOpenError extends ProtocolError {
  ...
  constructor(err) {
    super(
      err ||
        'An attempt was made to operate on a modal dialog when one ' +
          'was not open.',
...
export class ProxyRequestError extends ES6Error {
  constructor(err, responseError, httpStatus) {
    ...
    if (
      _.isPlainObject(responseErrorObj.value) &&
      _.has(responseErrorObj.value, 'error')
    ) {
  ...
  getActualError() {
    ...
    if (
      util.hasValue(this.jsonwp?.status) &&
      util.hasValue(this.jsonwp?.value)
    ) {
      return errorFromMJSONWPStatusCode(this.jsonwp.status, this.jsonwp.value);
    } else if (
      util.hasValue(this.w3c) &&
      _.isNumber(this.w3cStatus) &&
      this.w3cStatus >= 300
    ) {
...
function errorFromMJSONWPStatusCode(code, value = '') {
  ...
  if (code !== UnknownError.code() && jsonwpErrorCodeMap[code]) {
    mjsonwpLog.debug(
      `Matched JSONWP error code ${code} to ${jsonwpErrorCodeMap[code].name}`
    );
...
function errorFromW3CJsonCode(code, message, stacktrace = null) {
  if (code && w3cErrorCodeMap[code.toLowerCase()]) {
    w3cLog.debug(
      `Matched W3C error code '${code}' to ${
        w3cErrorCodeMap[code.toLowerCase()].name
      }`
    );
